...
def l2_cupy(X_cupy, Y_cupy):
    return float(cp.sum((X_cupy - Y_cupy) ** 2))

def l2_cupy_chunked(X_cupy, Y_cupy, chunk_size=10000000):
    ...
    for start_idx in range(0, n_elements, chunk_size):
        ...
        diff_squared = (chunk_X - chunk_Y) ** 2
        chunk_sum = cp.sum(diff_squared)
        total_sum += float(chunk_sum)
        
    return total_sum
        

def l2_pytorch(X_torch, Y_torch):
    return torch.sum((X_torch - Y_torch) ** 2)

@triton.jit
def l2_triton_atomic(X_ptr, Y_ptr, output_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    square_difference = (x - y) * (x - y)
    
    block_sum = tl.sum(square_difference, axis=0)
    tl.atomic_add(output_ptr, block_sum)

@triton.jit
def l2_triton_hierarchical_stage_1(X_ptr, Y_ptr, block_sums_ptr, n_elements, BLOCK_SIZE:tl.constexpr):
    pid = tl.program_id(axis=0)
    block_start = pid * BLOCK_SIZE
    offsets = block_start + tl.arange(0, BLOCK_SIZE)
...
@triton.jit    
def l2_triton_hierarchical_stage_2(block_sums_ptr, output_ptr, n_blocks, BLOCK_SIZE:tl.constexpr):
    pid = tl.program_id(axis=0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < n_blocks
    
    block_sums = tl.load(block_sums_ptr + offsets, mask=mask, other=0.0)
    final_sum = tl.sum(block_sums, axis=0)

    tl.atomic_add(output_ptr, final_sum)
...
def l2_sklearn(X, Y):
    ...
    dist = euclidean_distances(X, Y, squared=True)[0, 0]
    
    return float(dist)

def distance_l2_atomic(X_torch, Y_torch):
    ...
    l2_triton_atomic[grid](X_triton, Y_triton, output, n_elements, BLOCK_SIZE=block_size)
...
if __name__ == "__main__":
    ...
    start = time.time()
    reference = l2_sklearn(X_cpu, Y_cpu)
    end = time.time()
    reference_time = (end - start) * 1000 

    """Standard Cupy Implementation"""
    #warm_up_cupy_stardard
    l2_cupy(X_cupy, Y_cupy)
    cp.cuda.stream.get_current_stream().synchronize()
    
    #Benchmark Standard Cupy
    start = time.time()
    for _ in range(10):
        result_cupy_standard = l2_cupy(X_cupy, Y_cupy)
        cp.cuda.stream.get_current_stream().synchronize()
    end = time.time()
    ...
    l2_cupy_chunked(X_cupy, Y_cupy)
    cp.cuda.stream.get_current_stream().synchronize()
    
    #Benchmark Chunked Cupy
    start = time.time()
    for _ in range(10):
        result_cupy_chunked = l2_cupy_chunked(X_cupy, Y_cupy, chunk_size=10000000)
        cp.cuda.stream.get_current_stream().synchronize()
    end = time.time()
    ...
    l2_pytorch(X_torch, Y_torch)
    torch.cuda.synchronize()
    
    #Benchmark Torch Standard
    start = time.time()
    for _ in range(10):
        result_torch_standard = l2_pytorch(X_torch, Y_torch)
        torch.cuda.synchronize()
    end = time.time()
    ...
    result_triton_atomic, error_triton_atomic, time_triton_atomic = distance_l2_atomic(X_torch, Y_torch)
    ...
    l2_triton_hierarchical_stage_1[(n_blocks,)](X_triton, Y_triton, block_sums, n_elements, BLOCK_SIZE=block_size)
    if n_blocks > block_size:
        n_reduction_blocks = triton.cdiv(n_blocks, block_size)
        l2_triton_hierarchical_stage_2[(n_reduction_blocks,)](
            block_sums,
            output,
            n_blocks,
        ...
        l2_triton_hierarchical_stage_2[(1,)](
            block_sums,
            output,
            n_blocks,
            BLOCK_SIZE=block_size,
    ...
    print(f"Cupy Chuncked Result: {result_cupy_chunked} with the error of {error_cupy_chunked}. Time Performance: {time_cupy_chunked} ms.")
